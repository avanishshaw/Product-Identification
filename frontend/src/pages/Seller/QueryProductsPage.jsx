...
const QueryProductsPage = () => {
  ...
  return (
    <div className="card">
      <h2 className="text-2xl font-bold text-white mb-6">Products For Sale</h2>
      ...
      <div className="table-wrapper">
        <table>
          ...
          <tbody>
            {products.map(p => (<tr key={p._id}><td>{p.productId}</td><td>{p.name}</td><td>{p.brand}</td><td>${p.price}</td><td><span className="bg-blue-500/20 text-blue-300 px-2 py-1 rounded-full text-xs">{p.status}</span></td></tr>))}
          </tbody>
        </table>
        {products.length === 0 && <p className="p-4 text-center text-slate-400">No products to display. Enter a code and search.</p>}
      </div>
    </div>
  );
};
